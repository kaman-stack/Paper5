...
title( sprintf('$q_1 = %.1f$, $q_R=0.8$, $\\theta = %2.1f$ dB, $\\delta = %1.1f$', q1, 10*log(theta)/log(10), delta ), 'FontSize', FontSize,'FontWeight','bold', 'interpreter', 'latex');

legend( sprintf('$T_D$'), sprintf('$T_2$'),  'T',...
        'location', 'best','interpreter', 'latex', 'FontSize',FontSize, 'FontWeight','bold');
print('-depsc2', sprintf('T_ALL_vs_B_q1_%02d_theta_%1.0f_delta_%02d.eps', 10*q1, 10*log(theta)/log(10), 10*delta) );
